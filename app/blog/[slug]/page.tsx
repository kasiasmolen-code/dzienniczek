...
export default function BlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="lg:flex lg:h-screen lg:overflow-hidden bg-background">
      {/* Boczna nawigacja (desktop) — nie znika również w detalu wpisu */}
      <div className="hidden lg:block h-full">
        <DesktopSidebar />
      </div>

      <main className="flex-1 lg:overflow-y-auto px-6 md:px-8 pb-28 lg:pb-10">
        <div className="max-w-2xl mx-auto lg:mx-0">
          {/* Wstecz — tylko w detalu wpisu, prowadzi z powrotem do listy bloga */}
          ...
          {status === 'notfound' && <p className="text-muted-foreground">Wpis nie znaleziony.</p>}
          ...
          {status === 'ok' && article && (
            <article>
              ...
              <p className="text-sm text-muted-foreground mb-8">{formatDate(article.publishedAt)}</p>
